    ...
    const CONFIG = {
      ...
      avatar: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=800&auto=format&fit=crop&ixlib=rb-4.0.3&s=placeholder',
    ...
    const waLink = `https://wa.me/${wa}?text=${encodeURIComponent('Olá! Vi seu link na bio e quero saber mais sobre a oferta.')}`
    ...
    document.getElementById('buyBtn').addEventListener('click', ()=>{
      ...
      const url = CONFIG.defaultUrl + '/oferta?utm_source=bio&utm_medium=link&utm_campaign=sale'
    ...
    // modal lead
    function openForm(e){ document.getElementById('modal').style.display='flex' }
    ...
    document.getElementById('leadForm').addEventListener('submit', async function(ev){
      ...
      try{
        ...
        alert('Erro no envio — salve os dados manualmente enquanto configura o endpoint.')
      }
    })
    ...
    document.getElementById('copyBtn').addEventListener('click', async ()=>{
      ...
      try{ await navigator.clipboard.writeText(link); alert('Link copiado! Cole na bio do Instagram.')}catch(e){ prompt('Copie o link:', link) }
    })

    document.getElementById('shareBtn').addEventListener('click', ()=>{
      const link = CONFIG.defaultUrl
      if(navigator.share){ navigator.share({title: CONFIG.name, text: 'Confira:', url: link}).catch(()=>{window.open(link)}) }
      else window.open('https://wa.me/?text='+encodeURIComponent(link))
    })

    // dark mode toggle — simple: inverte variáveis (se quiser gerar arquivo css separado, faça isso)
    document.getElementById('dm').addEventListener('change', function(){
      if(this.checked){
        ...
        document.documentElement.style.setProperty('--muted','#556277')
        document.documentElement.style.setProperty('--glass','rgba(12,18,30,0.04)')
      } else {
...
document.getElementById('confirmBuyBtn').addEventListener('click', () => {
  const url = CONFIG.defaultUrl + '/oferta?utm_source=bio&utm_medium=modal&utm_campaign=sale'
  window.open(url, '_blank')
})
...
// ...existing code...
    ...
    // DICAS: você pode carregar links dinamicamente de /links.json e alterar o HTML conforme a necessidade
